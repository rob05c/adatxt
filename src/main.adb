...
procedure Main is
   ...
   Placeholder_Txt : Unbounded_String;
   Draw_Tmp        : Unbounded_String;
...
begin
   ...
   Window.Draw_Box (5, 5, 25, 25);

   Buffer_Pos := 0;
   Draw_Tmp := Window.Slice_Nth_Newline (Placeholder_Txt, Buffer_Pos);
   Window.Draw_Text_In_Box (Draw_Tmp, 6, 6, 24, 24, 1);

   loop
     Get_Immediate (Input_Ch);
      case Input_Ch is
         when 'q' =>
            Put (Consts.Ansi_Cursor_Show);
            Put (Consts.Ansi_Altbuf_Disable);
         ...
         when 'k' => -- vim up
            ...
            Draw_Tmp :=
              Window.Slice_Nth_Newline (Placeholder_Txt, Buffer_Pos);
            Window.Draw_Text_In_Box (Draw_Tmp, 6, 6, 24, 24, 1);
            -- Window.Draw_Text_In_Box (Draw_Tmp, 4, 4, 49, 99);
            -- Put (Consts.Ansi_Move_Cursor (31, 30));
            -- Put_Line ("got k");
         when 'j' => -- vim down
            -- TODO max buffer height? only the real newlines, must insert a newline?
            Buffer_Pos := Buffer_Pos + 1;
            Draw_Tmp := Window.Slice_Nth_Newline(Placeholder_Txt, Buffer_Pos);
            Window.Draw_Text_In_Box (Draw_Tmp, 6, 6, 24, 24, 1);
            ...
            -- Put_Line ("got k");
         when others =>
            Put (Consts.Ansi_Move_Cursor (30, 30));
            Put_Line ("Looping");
      end case;
   end loop;
